...
print(f"Dataset size={config.datasetNb}")
datasetAugmentedNb = config.datasetNb * 5
print(f"Dataset augmented size={datasetAugmentedNb}")
print(f"Test size 20%={config.datasetNb // 5}")
print(f"Training size={datasetAugmentedNb - config.datasetNb // 5}")
...
print(f"    Nb days={calcNbDays(config.datasetNb * config.datasetNb * 25088,config.datasetNb,1,nbcycle=1):.1f}")
...
d = calcNbDays(calcNbIter([],[4096,1024],10,25088)/1000, config.datasetNb)
...
d = calcNbDays(calcNbIter([],[3136,512],10,25088)/1000, config.datasetNb)
print(f"    Nb days={d:.1f} GPU={d / calcGpuRatio(1,25088):.1f}")
print(f"CVCTDK{16-2}-TCNN-VGG16-1H-10O")
print(f"    VGG16-CNN|512|{nbcat}")
print(f"    Nb kweigths={int(calcNbW([],[512],10,25088)/1000)}")
d = calcNbDays(calcNbIter([],[512],10,25088)/1000, config.datasetNb)
print(f"    Nb days={d:.1f} GPU={d / calcGpuRatio(1,25088):.1f}")
...
print(f"    Nb days={calcNbDays(config.datasetNb * config.datasetNb * 1056,config.datasetNb,1,nbcycle=1):.1f}")
...
d = calcNbDays(calcNbIter([],[1024],10,1056)/1000, config.datasetNb)
...
d = calcNbDays(calcNbIter([],[1000],10,1536)/1000, config.datasetNb)
...
d = calcNbDays(calcNbIter([32,64,128,256],[4096,1024],10,j,2,2), config.datasetNb)
...
d = calcNbDays(calcNbIter([32,32,64],[1024],10,j,2,2), config.datasetNb)
print(f"    Nb days={d:.1f} GPU={d / calcGpuRatio(j,12000):.1f}")
...
d = calcNbDays(calcNbIter([32,64],[512],10,j,3,2), config.datasetNb)
...
d = calcNbDays(calcNbIter([32,64],[256],10,j,3,1), config.datasetNb)
print(f"    Nb days={d:.1f} GPU={d / calcGpuRatio(j,9000):.1f}")
j = resdefectmin
...
d = calcNbDays(calcNbIter([],[128],10,(j**2)*1), config.datasetNb)
...
d = calcNbDays(calcNbIter([32,64],[512],10,j,2,1), config.datasetNb * 16)
...
d = calcNbDays(calcNbIter([32,64],[512],10,j,3,1), config.datasetNb * 16)
print(f"    Nb days={d:.1f} GPU={d / calcGpuRatio(j,5000):.1f}")
...
d = calcNbDays(calcNbIter([32],[512],10,j,3,1), config.datasetNb * 16)
print(f"    Nb days={d:.1f} GPU={d / calcGpuRatio(j,5000):.1f}")
...
d = calcNbDays(calcNbIter([32],[256],10,j,4,1), config.datasetNb * 16)
print(f"    Nb days={d:.1f} GPU={d / calcGpuRatio(j,5000):.1f}")
j = resdefectmin
print(f"CVCTDK{1+2+1}-MLP-{(j**2)*1}I-1H-10O") # Concat layers not flatten
print(f"    {j}*{j}*1|{(j**2)*1}|128|{nbcat}")
